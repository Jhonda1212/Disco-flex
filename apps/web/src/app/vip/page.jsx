...
const SALAS = [
  {
    ...
    capacidad: 10,
    precio_hora: 150,
    descripcion: 'Ambiente íntimo con decoración premium. Perfecta para grupos reducidos.',
    disponible: true,
  },
  {
    id: 'negra',
    ...
    capacidad: 15,
    precio_hora: 200,
    descripcion: 'La sala más exclusiva. Sonido privado y servicio dedicado.',
    disponible: false,
  },
  {
    id: 'dorada',
    ...
    capacidad: 20,
    precio_hora: 300,
    descripcion: 'La experiencia Flex definitiva. Diseño dorado, bar privado y DJ dedicado.',
    disponible: true,
  },
]
...
export default function PaginaVIP() {
  ...
  const puedeReservar = salaSeleccionada && fecha && hora && duracion && sala?.disponible
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-3 gap-4 mb-10">
        {SALAS.map(s => (
          <button
            key={s.id}
            onClick={() => s.disponible && setSalaSeleccionada(s.id)}
            disabled={!s.disponible}
            className={`bg-linear-to-br ${s.color} border ${s.borde} rounded-2xl p-6 text-left transition-all ${
              salaSeleccionada === s.id
                ? 'ring-2 ring-gold-500 scale-[1.02]'
                : s.disponible ? 'hover:scale-[1.01] cursor-pointer' : 'opacity-50 cursor-not-allowed'
            }`}
          >
            <div className="flex items-start justify-between mb-4">
              <Crown size={24} className={s.acento} />
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                s.disponible ? s.badge : 'bg-zinc-700 text-zinc-500'
              }`}>
                {s.disponible ? 'Disponible' : 'Ocupada'}
              </span>
            </div>
            <h3 className={`text-lg font-bold mb-1 ${s.acento}`}>{s.nombre}</h3>
